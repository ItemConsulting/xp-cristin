...
function addTypeScriptSupport(cfg) {
  ...
  return R.pipe(setEntry("ts/bundle", "./ts/main.ts"), addRule(rule), prependExtensions([".tsx", ".ts", ".json"]))(cfg);
}
...
function addSassSupport(cfg) {
  const rule = {
    test: /\.(sass|scss)$/,
    use: [...createDefaultCssLoaders(), { loader: "sass-loader", options: { sourceMap: !isProd } }],
  };

  const plugin = createCssPlugin();

  return R.pipe(addRule(rule), addPlugin(plugin), appendExtensions([".sass", ".scss", ".css"]))(cfg);
}
...
module.exports = R.pipe(addBabelSupport, addTypeScriptSupport, addSassSupport, addFontSupport)(config);
